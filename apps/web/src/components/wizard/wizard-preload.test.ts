...
const doc = { id: 'doc_1', title: 'Employment contract' } as unknown as DocumentSummary;
...
describe('preloadedWizardState', () => {
  ...
  it('runs the email tail recipients → review exactly like the from-scratch path', () => {
    ...
    const valid: RecipientDraft = { id: 'r1', email: 'signer@example.com', name: 'Jane Hong' };
  ...
  it('drops the template fields when the source PDF is re-uploaded', () => {
    let state = preloadedWizardState(preload());
    const newDoc = { id: 'doc_2', title: 'Re-upload' } as unknown as DocumentSummary;
